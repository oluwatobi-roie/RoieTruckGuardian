...
from config import PRIMARY_EMAIL, SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASSWORD, ALERT_EMAILS
...
def build_email(viol, Plate_Number):
    subject = f"{BRAND} – Violation Alert for Truck {safe(viol.device_id)} {Plate_Number}"

    text_body = f"""
Violation Alert – {BRAND}

Truck: {safe(viol.device_id)} {Plate_Number}
Violation ID: {safe(viol.id)}
...
<body style="font-family:Arial, sans-serif; color:#333; line-height:1.5;">
    <div style="max-width:600px; margin:auto; padding:20px; border-radius:10px;
        ...
        <p><strong>Truck ID:</strong> {safe(viol.device_id)} {Plate_Number}</p>
        Violation ID: {safe(viol.id)}
...
# ============================
async def send_alert(viol, Plate_Number):
    msg = build_email(viol, Plate_Number)
