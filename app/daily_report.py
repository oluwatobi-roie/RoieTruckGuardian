...
def to_dt(s: str) -> datetime:
    # YYYY-MM-DD → midnight Nigeria time
    if len(s) == 10:
        dt = datetime.fromisoformat(s)
        return dt.replace(tzinfo=NIGERIA_TZ)

    # ISO datetime → assume Nigeria time if naive
    dt = datetime.fromisoformat(s)
...
def strip_tz(df):
    for col in df.select_dtypes(include=['datetimetz']).columns:
        df[col] = df[col].dt.tz_convert("Africa/Lagos").dt.tz_localize(None)
    return df

def localize_df(df):
    """Assign Africa/Lagos timezone to all naive datetimes."""
    for col in df.select_dtypes(include=['datetime64[ns]']).columns:
        df[col] = df[col].dt.tz_localize(NIGERIA_TZ)
    return df
...
def compute_stops_from_positions(positions_df, min_stop_minutes=10):
    """
    Computes stops per device from raw positions dataframe.

    Rules:
    - speed < 3 km/h = considered potential stop
    - must remain stopped for >= min_stop_minutes to count
    - returns a DataFrame with columns:
        id, device_id, stop_time, resume_time, duration_s,
        lat, lon, address, zone
    """

    if positions_df.empty:
        return pd.DataFrame()

    positions_df = positions_df.sort_values(["device_id", "fix_time"])

    all_stops = []
    stop_id = 1  

    for dev, group in positions_df.groupby("device_id"):
        ...
        for _, row in group.iterrows():
            is_stopped = (row.get("speed", 0) < 3)

            if is_stopped and not in_stop:
                # stopping begins
                in_stop = True
                stop_start = row["fix_time"]
                last_stop_point = row

            elif is_stopped and in_stop:
                # continuing stop
                last_stop_point = row

            elif not is_stopped and in_stop:
                # stop ends here
                stop_end = row["fix_time"]
                duration_s = (stop_end - stop_start).total_seconds()

                if duration_s >= min_stop_minutes * 60:
                    all_stops.append({
                        "id": stop_id,
                        "device_id": dev,
                        "stop_time": stop_start,
                        "resume_time": stop_end,
                        "duration_s": duration_s,
                        "lat": last_stop_point.get("latitude"),
                        "lon": last_stop_point.get("longitude"),
                        "address": last_stop_point.get("address"),
                        "zone": last_stop_point.get("zone"),
                    })
                    stop_id += 1
        ...
        # If day ends while still stopped
        if in_stop:
            stop_end = group.iloc[-1]["fix_time"]
            duration_s = (stop_end - stop_start).total_seconds()
...
def build_excel(by_device, fleet_summary, positions_df, trips_df, stops_df, violations_df, out_prefix):
    ...
    with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
        ...
        # =====================================================
        # 4. VEHICLE STORY SHEET (your required format)
        # =====================================================

        combined_sheet = "Trips & Violations"
        story_sheet = "Vehicle Story"
        ws = writer.book.create_sheet(story_sheet)

        # Excel column headers
        ws.append(["Plate Number", "Status", "Information", "Violation/Observation"])

        for dev, m in by_device.items():
            t = m.get('truck')
            truck_name = t.name if t else f"Device {dev}"

            # Write vehicle header row (bold)
            ws.append([truck_name, "", "", ""])
            header_row = ws.max_row
            for col in range(1, 5):
                ws.cell(row=header_row, column=col).font = Font(bold=True)

            # Get all TRIPS + STOPS for this device
            dev_trips = trips_df[trips_df["device_id"] == dev].copy()
            dev_stops = stops_df[stops_df["device_id"] == dev].copy()
            ...
            for _, r in dev_trips.iterrows():
                trip_events.append({
                    ...
                    "start_zone": r.get("start_zone")
                })
                trip_events.append({
                    "time": r["end_time"],
                    "type": "trip_end",
                    "end_addr": r.get("end_addr"),
                    "end_zone": r.get("end_zone")
                })

            # Convert stops to events
            stop_events = []
            for _, s in dev_stops.iterrows():
                stop_events.append({
                    "time": s["stop_time"],
                    "type": "stop_start",
                    "addr": s.get("stop_addr"),
                    "zone": s.get("start_zone")
                })
                stop_events.append({
                    "time": s["resume_time"],
                    "type": "stop_end",
            ...
            all_events = trip_events + stop_events
            all_events = [e for e in all_events if e["time"] is not None]
            all_events.sort(key=lambda x: x["time"])

            # Add violation info for this device
            dev_viol = violations_df[violations_df["device_id"] == dev]

            def violation_at_time(t):
                """Return violation address if found around a stop time."""
                row = dev_viol[
                    (dev_viol["detected_at"] >= t - pd.Timedelta(minutes=2)) &
                    (dev_viol["detected_at"] <= t + pd.Timedelta(minutes=2))
                ]
                if not row.empty:
                    r = row.iloc[0]
                    return f"Parked at unapproved location ({r['address']})"
                return ""

            last_trip_start_addr = None

            # BUILD STORY LINES
            for ev in all_events:
                tstamp = ev["time"].strftime("%I:%M %p").lower()

                if ev["type"] == "stop_start":
                    # Check violation
                    viol = violation_at_time(ev["time"])
                    ws.append([
                        truck_name,
                        "Parked",
                        ev.get("addr") or ev.get("zone"),
                        viol
                    ])

                elif ev["type"] == "trip_start":
                    last_trip_start_addr = ev.get("start_addr") or ev.get("start_zone")
                    ws.append([truck_name, "Start Driving", tstamp, ""])

                elif ev["type"] == "trip_end":
                    end_addr = ev.get("end_addr") or ev.get("end_zone")
                    info = f"{last_trip_start_addr} > {end_addr}"
                    ws.append([truck_name, "Driving", info, ""])

                    ws.append([truck_name, "Stop Driving", tstamp, ""])

            # Add blank line after each vehicle
            ws.append(["", "", "", ""])
...
def main():
    ...
    # ----- 1. Parse input times in Africa/Lagos -----
    start_dt = to_dt(args.start)
    end_dt   = to_dt(args.end)

    if not args.db:
        raise SystemExit("Please set DATABASE_URL or pass --db")

    engine = create_engine(args.db, future=True)

    print(f"Fetching data between {start_dt} and {end_dt} (Africa/Lagos)...")

    # ----- 2. Fetch raw objects -----
    truck_map, positions, trips, stops, violations = fetch_data(
        engine, start_dt, end_dt
    )
    ...
    # ----- 4. Localize timestamps to Africa/Lagos -----
    positions_df = localize_df(positions_df)
    trips_df = localize_df(trips_df)
    ...
    violations_df = pd.DataFrame([{
        ...
        'lon': v.lon,
        'address': v.address,
        'minutes_in_unapproved': v.minutes_in_unapproved,
        'active': v.active
    } for v in violations]) if violations else pd.DataFrame()

    if not violations_df.empty:
        violations_df = localize_df(violations_df)

    # ----- 5. Compute stops automatically -----
    print("Computing stops from positions…")
    stops_df = compute_stops_from_positions(positions_df)

    if not stops_df.empty:
        stops_df = localize_df(stops_df)

    # ----- 6. Compute metrics -----
    print("Computing metrics…")
    by_device, fleet_summary = compute_metrics(
        truck_map,
        positions_df,
        trips_df,
        stops_df,
    ...
    # ----- 7. Strip timezone JUST BEFORE Excel writing -----
    positions_out = strip_tz(positions_df.copy())
    trips_out = strip_tz(trips_df.copy())
    stops_out = strip_tz(stops_df.copy())
    viol_out = strip_tz(violations_df.copy()) if not violations_df.empty else violations_df
